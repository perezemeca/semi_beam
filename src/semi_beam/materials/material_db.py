...
def default_materials_path() -> Path:
    """
    Ruta del TXT de materiales con fallback para desarrollo y PyInstaller.
    """
    here = Path(__file__).resolve()
    candidates: List[Path] = []

    # Desarrollo (repo)
    candidates.append(here.parents[1] / "data" / "materials_kgcm2.txt")
    candidates.append(here.parents[2] / "semi_beam" / "data" / "materials_kgcm2.txt")

    # PyInstaller one-file (extraído en _MEIPASS)
    meipass = getattr(sys, "_MEIPASS", None)
    if meipass:
        base = Path(meipass)
        candidates.append(base / "semi_beam" / "data" / "materials_kgcm2.txt")
        candidates.append(base / "src" / "semi_beam" / "data" / "materials_kgcm2.txt")
